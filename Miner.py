...
username = "docent1k" # Edit this to your username, mind the quotes
...
while True:
    ...
    for result in range(100 * int(difficulty) + 1): # Calculate hash with difficulty
        ...
        if job[1] == ducos1: # If result is even with job
            soc.send(bytes(str(result)+",,GitHub Actions", encoding="utf8")) # Send result of hashing algorithm to pool
